...
private:
    ...
    void Volumecontrol() {
        int VolumeButton_adc_value;
        ESP_ERROR_CHECK(adc_oneshot_read(adc_handle_, POWER_VOLUME_ADC_CHANNEL, &VolumeButton_adc_value));
        // ESP_LOGI("VolumeButton_adc_value", "VolumeButton ADC value: %d ", VolumeButton_adc_value);
        if ( 1100 < VolumeButton_adc_value && VolumeButton_adc_value < 2800) {
        ...
        if ( 50 < VolumeButton_adc_value && VolumeButton_adc_value < 1000) {
            auto& board = Board::GetInstance();
            auto display = board.GetDisplay();
            auto codec = board.GetAudioCodec();
            auto volume = codec->output_volume() - 10;
    ...
    void CheckBatteryStatus() {
        ...
        bool new_charging_status = (1500 < usb_usb_adc_value0 && usb_usb_adc_value0 < 4000);
...
public:
    PowerManager(gpio_num_t pin) : charging_pin_(pin) {
        ...
        ESP_ERROR_CHECK(adc_oneshot_config_channel(adc_handle_, POWER_VOLUME_ADC_CHANNEL, &chan_config)); // 音量键
