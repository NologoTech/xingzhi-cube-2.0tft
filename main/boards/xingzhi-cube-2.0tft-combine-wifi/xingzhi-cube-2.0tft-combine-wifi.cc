...
#define TAG "XINGZHI_CUBE_2_0TFT_COMBINE_WIFI"

class XINGZHI_CUBE_2_0TFT_COMBINE_WIFI : public WifiBoard {
private:
    Button boot_button_;
    LcdDisplay* display_;
    ...
    void InitializePowerSaveTimer() {
        power_save_timer_ = new PowerSaveTimer(-1, 12, -1);
    ...
    void InitializeButtons() {
        ...
        ESP_LOGI(TAG, "Set the currently running firmware to Valid");
        ...
        boot_button_.OnLongPress([this]() {
            ...
            if (!WifiStation::GetInstance().IsConnected()) {
                display_->SetStatus(Lang::Strings::SWITCH_SYSTEMS_BEFORE);
                return;
            }
            display_->SetStatus(Lang::Strings::SWITCH_SYSTEMS);
            ...
            while (app.GetDeviceState() != kDeviceStateIdle && is_listeningorspreaking) {
                ESP_LOGI(TAG, "Wait for it to be back on standby before shutting down...");
                app.ToggleChatState(); // 切换状态
                vTaskDelay(pdMS_TO_TICKS(50));
            }
            vTaskDelay(pdMS_TO_TICKS(300));
            Ota ota;  // 创建Ota类的实例
            ota.switchfirmware();  // 调用switchfirmware方法
        });
    }
...
public:
    XINGZHI_CUBE_2_0TFT_COMBINE_WIFI() :
...
DECLARE_BOARD(XINGZHI_CUBE_2_0TFT_COMBINE_WIFI);
